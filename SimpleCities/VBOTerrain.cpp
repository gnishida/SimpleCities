...
VBOTerrain::VBOTerrain() {
	...
	texId = 0;
	//_resolution = 200;
	size = glm::vec2(1000, 1000);
}

void VBOTerrain::init(VBORenderManager* rendManager, const glm::vec2& size) {
...
float VBOTerrain::getTerrainHeight(float u, float v) {
	...
	int c1 = u * size.x;// _resolution;
	int c2 = u * size.x + 1;// _resolution + 1;
	int r1 = v * size.y;// _resolution;
	int r2 = v * size.y + 1;// _resolution + 1;
...
void VBOTerrain::loadTerrain(const QString& fileName) {
	gs::DEM dem;
	dem.load(fileName.toUtf8().constData());

	rendManager->changeTerrainDimensions(glm::vec2(dem.width, dem.height));
	layerData = cv::Mat(dem.height, dem.width, CV_32FC1, dem.data.data()).clone();
